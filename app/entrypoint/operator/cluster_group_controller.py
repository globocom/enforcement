...
class ClusterGroupController(BaseController):
    ...
    def update(self, old: List[dict], new: List[dict], status: dict, **kwargs):
        ...
        old_enforcement_list = ClusterGroupController._make_enforcement_list(old)
        new_enforcement_list = ClusterGroupController._make_enforcement_list(new)
    ...
    def register(self):
        ...
        self.register_method(kopf.on.timer, self.sync, self.KIND, id='sync', interval=8, initial_delay=15, idle=15)
